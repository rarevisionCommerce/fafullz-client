...
function Accounts() {
  ...
  const fetchAccounts = () => {
    return axios.get(
      `/accounts?page=${activePage}&perPage=${perPage}&category=${category}&state=${state}&country=${country}&sellerId=${sellerId}`
    );
  };
  ...
  } = useQuery(["accounts", activePage], fetchAccounts, {
    refetchOnWindowFocus: true,
    keepPreviousData: true,
  });
  // pagination refetch
  const totalPages = Math.ceil(accountsData?.data?.count / perPage);

  useEffect(() => {
    // Invalidate the "user" query with the new userId when the name changes
    refetch();
  }, [activePage, category, state, country, sellerId, perPage]);
  ...
  return (
    <div className="bg-light min-h-screen py-4 px-1 md:px-6 shadow-md">
      <h1 className="mb-4 text-center font-bold  ">Accounts </h1>
      <div className="flex flex-col ">
        ...
        >
          ...
          <div className="flex justify-between mt-5 px-1 md:px-4 ">
            <h1>Total: {accountsData?.data?.count || 0}</h1>
            <h1
        ...
        <div className="overflow-y-auto my-3 rounded-sm">
          ...
          <table className="w-full table-auto border-collapse border border-slate-500 text-light text-sm">
            ...
            <tbody className="text-dark">
              {loadingAccounts || refetchingAccounts ? (
                <tr className="flex justify-center py-4 pr-6 items-center">
                  <td colSpan={10} className="text-center pl-[200px]">
                    <PulseLoader color="#6ba54a" size={10} />
                  </td>
                </tr>
              ) : accountsData?.data?.message ? (
                <tr>
                  <td
                    colSpan={6}
                    className="border-collapse text-center border-b border-slate-500 py-2 px-3"
                  >
                    {accountsData?.data?.message}
                  </td>
                </tr>
              ) : (
                accountsData?.data?.accounts?.map((account, index) => {
                  return (
                    <tr
                      key={index}
                    ...
                    >
                      ...
                      {
                          loadingAccounts || loadingUserCart ?
                          <span>
