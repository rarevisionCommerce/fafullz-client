...
function AccountProducts(props) {
  ...
  const fetchProducts = () => {
    return axios.get(
      `/accounts/all/${auth?.jabberId}?page=${activePage}&perPage=${perPage}&status=${status}&isPaid=${isPaid}`
    );
  };
  ...
  } = useQuery([`accounts-${auth?.jabberId}`, activePage, perPage, status, isPaid], fetchProducts, {
  ...
  const { isLoading: isDeleting, mutate: deleteProductMutate } = useMutation(
    deleteProduct,
    {
      onSuccess: (response) => {
        ...
        toast.success(text);
        queryClient.invalidateQueries([`accounts-${auth?.jabberId}`]);
        closeDelete();
      },
      onError: (err) => {
  ...
  return (
    <div className="min-h-screen">
         ...
         <div className="overflow-x-auto">
             <Table striped highlightOnHover withBorder withColumnBorders style={{ color: '#d1d5db' }}>
                ...
                <tbody>
                    ...
                    ) : !productData?.data?.accounts || productData?.data?.accounts?.length < 1 ? (
                        <tr>
                          <td colSpan={5} align="center" style={{ color: '#9ca3af', padding: '20px' }}>
                            No Account orders
                          </td>
                        </tr>
                    ) : (
                        productData?.data?.accounts?.map((item, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{item?.sitename}</td>
      ...
      <Modal 
        opened={deleteOpened} 
        onClose={closeDelete} 
        title={<Text color="white">Delete Account</Text>} 
        centered
        styles={{ 
