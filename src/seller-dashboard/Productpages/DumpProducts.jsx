...
function DumpProducts() {
  ...
  const fetchProducts = () => {
    return axios.get(
      `/dumps/all/${auth?.jabberId}?page=${activePage}&perPage=${perPage}&status=${status}&isPaid=${isPaid}`
    );
  };
  ...
  } = useQuery([`dumps-${auth?.jabberId}`, activePage, perPage, status, isPaid], fetchProducts, {
  ...
  const { isLoading: isDeleting, mutate: deleteProductMutate } = useMutation(
    deleteProduct,
    {
      onSuccess: (response) => {
        ...
        toast.success(text);
        queryClient.invalidateQueries([`dumps-${auth?.jabberId}`]);
        closeDelete();
      },
      onError: (err) => {
  ...
  return (
    <div className="min-h-screen">
             ...
             <Table striped highlightOnHover withBorder withColumnBorders style={{ color: '#d1d5db' }}>
                ...
                <tbody>
                    {loadingProducts || refetchingProducts ? (
                        <tr>
                          <td colSpan={12} align="center" style={{ padding: '20px' }}>
                            <Loader color="green" variant="dots" />
                          </td>
                        </tr>
                    ) : !productData?.data?.files || productData?.data?.files?.length < 1 ? (
                        <tr>
                          <td colSpan={12} align="center" style={{ color: '#9ca3af', padding: '20px' }}>
                            No Dump products
                          </td>
                        </tr>
                    ) : (
      ...
      <Modal 
        opened={deleteOpened} 
        onClose={closeDelete} 
        title={<Text color="white">Delete Dump</Text>} 
        centered
        styles={{ 
