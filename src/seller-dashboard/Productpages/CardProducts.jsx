...
function CardProducts() {
  ...
  } = useQuery([`cards-${auth?.jabberId}`, activePage, perPage, status, isPaid], fetchProducts, {
  ...
  return (
    <div className="min-h-screen">
             ...
             <Table striped highlightOnHover withBorder withColumnBorders style={{ color: '#d1d5db' }}>
                ...
                <tbody>
                    {loadingProducts || refetchingProducts ? (
                        <tr>
                          <td colSpan={25} align="center" style={{ padding: '20px' }}>
                            <Loader color="green" variant="dots" />
                          </td>
                        </tr>
                    ) : !productData?.data?.cards || productData?.data?.cards?.length < 1 ? (
                        <tr>
                          <td colSpan={25} align="center" style={{ color: '#9ca3af', padding: '20px' }}>
                            No Card products
                          </td>
                        </tr>
                    ) : (
      ...
      <Modal 
        opened={deleteOpened} 
        onClose={closeDelete} 
        title={<Text color="white">Delete Card</Text>} 
        centered
        styles={{ 
