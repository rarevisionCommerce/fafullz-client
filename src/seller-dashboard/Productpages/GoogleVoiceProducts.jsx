...
function GoogleVoiceProducts(props) {
  ...
  const fetchProducts = () => {
    return axios.get(
      `/gVoices/all/${auth?.jabberId}?page=${activePage}&perPage=${perPage}&status=${status}&isPaid=${isPaid}`
    );
  };
  ...
  } = useQuery([`gVoices-${auth?.jabberId}`, activePage, perPage, status, isPaid], fetchProducts, {
  ...
  // pagination refetch is handled key dependency
  ...
  const { isLoading: isDeleting, mutate: deleteProductMutate } = useMutation(
    deleteProduct,
    {
      onSuccess: (response) => {
        ...
        toast.success(text);
        queryClient.invalidateQueries([`gVoices-${auth?.jabberId}`]);
        closeDelete();
      },
      onError: (err) => {
  ...
  return (
    <div className="min-h-screen">
         ...
         <div className="overflow-x-auto">
             <Table striped highlightOnHover withBorder withColumnBorders style={{ color: '#d1d5db' }}>
                ...
                <tbody>
                    ...
                    ) : !productData?.data?.gVoices || productData?.data?.gVoices?.length < 1 ? (
                        <tr>
                          <td colSpan={7} align="center" style={{ color: '#9ca3af', padding: '20px' }}>
                            No Google voice products
                          </td>
                        </tr>
                    ) : (
                        productData?.data?.gVoices?.map((item, index) => (
                            <tr key={index}>
                                <td>{index + 1}</td>
                                <td>{item?.email}</td>
                                <td>{item?.password}</td>
                                <td>{item?.state}</td>
                                <td>{item?.recoveryMail}</td>
                                <td>${formatCurrency(item?.price?.price)}</td>
                                <td>
                                    <ActionIcon 
                                        color="red" 
                                        variant="filled"
      ...
      <Modal 
        opened={deleteOpened} 
        onClose={closeDelete} 
        title={<Text color="white">Delete Google Voice</Text>} 
        centered
        styles={{ 
...
export default GoogleVoiceProducts;
