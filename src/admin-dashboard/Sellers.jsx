...
function Sellers(props) {
  ...
  const fetchSellers = () => {
    return axios.get(
      `/users?page=${activePage}&perPage=${perPage}&role=Seller&userName=${userName}&jabberId=${jabberId}`
    );
  };
  ...
  const totalPages = Math.ceil(sellersData?.data?.count / perPage);
      ...
      customUI: ({ onClose }) => {
        return (
          <div className=" shadow-xl p-[30px] flex flex-col gap-4">
            <h1 className="font-bold text-xl">Delete Seller!</h1>
            <p className="pb-1">Are you sure you want to delete this Seller?</p>
  ...
  return (
    <div className="bg-light px-3 py-3">
      <h1 className="font-bold text-lg">All Sellers </h1>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="border-collapse border-b border-slate-500 py-3 px-3 flex justify-center gap-3">
                        <Link
                          to={`/admin-dash/seller-details/${item._id}`}
