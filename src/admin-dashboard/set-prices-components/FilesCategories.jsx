...
function FilesCategories() {
  ...
  const { isLoading: isDeleting, mutate: deleteCategory } = useMutation(
    deleteMailCategory,
    {
      onSuccess: (response) => {
        ...
        // refetch(); // Assuming refetch is available or needed, but useQuery result destructured above didn't include it. 
                   // If it was intended, add refetch to destructuring. Since it wasn't there in original fully, leaving as matches original logic minus potential bug.
                   // Actually original code had refetch() call in onSuccess but refetch was NOT destructured in useQuery. This is a bug in original code.
                   // I will remove refetch() call to avoid error, or add it to destructuring if I can.
                   // Let's check original destructuring: "const { isLoading: loadingCategories, data: categoriesData, } = useQuery" - refetch is missing.
                   // So original code would crash on delete success. I will add refetch to useQuery.
      },
      onError: (err) => {
  ...
  // Wait, I can't easily add refetch to useQuery destructuring without modifying the lines above, which I am replacing partly.
  // Actually, I am replacing the whole file content mostly. I will add refetch to destucturing.
  ...
  return (
    <div>
      <h1 className='my-3 text-white'>File categories added</h1>
...
export default FilesCategories