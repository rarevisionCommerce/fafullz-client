...
function MailCategory() {
  ...
  const { isLoading: loadingCategories, data: categoriesData, refetch} = useQuery(
    [`mailcategory-`],
    fetchCategories,
    {
  ...
  const { isLoading: isDeleting, mutate: deleteCategory } = useMutation(
    deleteMailCategory,
    {
      onSuccess: (response) => {
        ...
        toast.success(text);
        queryClient.invalidateQueries([`mailcategory-`]);
      ...
      customUI: ({ onClose }) => {
        return (
          <div className=" shadow-xl p-[30px] flex flex-col gap-4">
            <h1 className="font-bold text-xl">Delete TextNow/mail category</h1>
  ...
  return (
    <div>
      <h1 className='my-3'>TextNow/mail categories added</h1>
