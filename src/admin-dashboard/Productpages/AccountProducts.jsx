...
import { Alert, Pagination } from "@mantine/core";
...
function AccountProducts(props) {
  ...
  const fetchProducts = () => {
    return axios.get(
      `/accounts/all/${props?.jabberId}?page=${activePage}&perPage=${perPage}&status=${status}&isPaid=${isPaid}`
    );
  };
  ...
  } = useQuery([`accounts-${auth?.jabberId}`, activePage], fetchProducts, {
          ...
          <table className="w-full text-center table-auto border-collapse border border-slate-500 text-light text-sm">
            ...
            <tbody className="text-dark">
              ...
              loadingProducts || refetchingProducts ? 
               ...
               : 
              !productData?.data?.accounts ||
              productData?.data?.accounts?.length < 1 ? 
                <tr>
