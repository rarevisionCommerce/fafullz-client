...
function DumpProducts(props) {
  ...
  const fetchProducts = () => {
    return axios.get(
      `/dumps/all/${props?.jabberId}?page=${activePage}&perPage=${perPage}&status=${status}&isPaid=${isPaid}`
    );
  };
  ...
  const { mutate: payOneProductMutate, isLoading: loadingPayOne } = useMutation(
    payOneProduct,
    {
      onSuccess: (response) => {
        ...
        toast.success(text)
        queryClient.invalidateQueries([`dumps-${auth?.jabberId}`, activePage]);
      },
      onError: (err) => {
